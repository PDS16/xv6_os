...
int emufs_delete(int dir_handle, char* path){
    ...
    */
    ...
    int inode_number = return_inode(dir[dir_handle].mount_point,dir[dir_handle].inode_number,path);
    if( inode_number == -1) return -1;
    int parent_inode_number = delete_entity(dir[dir_handle].mount_point,inode_number);
    read_inode(dir[dir_handle].mount_point,parent_inode_number,&parent_inode);
    int i=0;
    for (i = 0; i < parent_inode.size; i++)
    {
        if ( parent_inode.mappings[i] == inode_number) break;
    }
    for ( int j=i+1;j<parent_inode.size;j++)
    {
        parent_inode.mappings[j-1] = parent_inode.mappings[j];
    }
    parent_inode.mappings[parent_inode.size-1] = 0;
    parent_inode.size--;
    write_inode(dir[dir_handle].mount_point,parent_inode_number,&parent_inode);
    return 1;
}
...
int emufs_create(int dir_handle, char* name, int type){
    ...
    */
        ...
        for (size_t i = 0; i < 4; i++)
        {
            ...
            // printf("Input key: 2");
            read_inode(dir[dir_handle].mount_point,parent_inode.mappings[i],&child_inode);
            if(strcmp(child_inode.name,name) == 0 && child_inode.type == type) return -1;
        }
        if ( children == 4) return -1;
        int index = 0;
        if ( type == 1) index = alloc_dir_handle();
        else index = alloc_file_handle();
        int inode_number = alloc_inode(dir[dir_handle].mount_point);
        if(index == -1 || inode_number == -1) return -1;
        parent_inode.mappings[found] = inode_number;
        parent_inode.size++;
        struct inode_t new_inode;
        read_inode(dir[dir_handle].mount_point, inode_number, &new_inode);
        ...
        for (size_t i = 0; i < 4; i++)
        {
            new_inode.mappings[i] = 0     ;
        }
        write_inode(dir[dir_handle].mount_point, inode_number, &new_inode);
...
int open_file(int dir_handle, char* path){
    ...
    */
    ...
    files[file_handle].inode_number = inode_num;
...
int emufs_read(int file_handle, char* buf, int size){
    /*
        * Read the file into buf starting from j(offset) 
    ...
    */
    ...
    if ( files[file_handle].offset + size > inode.size) return -1;
    int end_offset = BLOCKSIZE-1;
    int start_offset = files[file_handle].offset%BLOCKSIZE;
    int start_block = files[file_handle].offset/BLOCKSIZE;
    int end_block = (files[file_handle].offset + size-1)/BLOCKSIZE;
    int curr_block = files[file_handle].offset/BLOCKSIZE;
    int bytes_read = 0;
    char temp_buf[BLOCKSIZE];
    // printf("%d read2 %d %d %d\n",start_offset,end_offset,start_block,files[file_handle].offset );    
    // fflush(stdout);
    
    while(curr_block != end_block+1)
    {
        ...
        {
            end_offset = (files[file_handle].offset + size-1)%BLOCKSIZE;
        }
        read_datablock(files[file_handle].mount_point,inode.mappings[curr_block],temp_buf);
        memcpy(buf+bytes_read,temp_buf+start_offset,end_offset-start_offset+1);
        curr_block++;
        bytes_read += end_offset-start_offset+1;
    }
...
int emufs_write(int file_handle, char* buf, int size){
    ...
    */
    struct inode_t inode;
    read_inode(files[file_handle].mount_point,files[file_handle].inode_number,&inode);
    if ( files[file_handle].offset + size > 4*BLOCKSIZE) return -1;
    int end_offset = BLOCKSIZE-1;
    int start_offset = files[file_handle].offset%BLOCKSIZE;
    int start_block = files[file_handle].offset/BLOCKSIZE;
    int end_block = (files[file_handle].offset + size-1)/BLOCKSIZE;
    ...
    while(curr_block != end_block+1)
    {
        ...
        if(inode.mappings[curr_block] == 0) 
        {
            int block_number =  alloc_datablock(files[file_handle].mount_point);
            if ( block_number == -1) return -1;
            inode.mappings[curr_block] = block_number;
            inode.size+=BLOCKSIZE;
            write_inode(files[file_handle].mount_point,files[file_handle].inode_number,&inode);
        }    
        read_datablock(files[file_handle].mount_point,inode.mappings[curr_block],temp_buf);
        memcpy(temp_buf+start_offset,buf+bytes_read,end_offset-start_offset+1);
        write_datablock(files[file_handle].mount_point,inode.mappings[curr_block],temp_buf);
        curr_block++;
        bytes_read += end_offset-start_offset+1;
    }
...
int emufs_seek(int file_handle, int nseek){
    ...
    */
    struct inode_t inode;
    read_inode(files[file_handle].mount_point,files[file_handle].inode_number,&inode);
    if ( files[file_handle].offset + nseek> inode.size || files[file_handle].offset + nseek< 0) return -1;
