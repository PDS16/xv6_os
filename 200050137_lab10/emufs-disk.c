...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	/*
		* Reads the metadata in block 2 or 
		3
	...
	*/
	struct metadata_t buf;
	int fd = mounts[mount_point].device_fd; 
	if(inodenum < 16)readblock(fd,1,(char *)&buf);
	else readblock(fd,2,(char *)&buf);
	memcpy(inodeptr,&(buf.inodes[inodenum%16]),sizeof(struct inode_t));
}
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	memcpy(&(buf.inodes[inodenum%16]),inodeptr,sizeof(struct inode_t));
	if(inodenum < 16)writeblock(fd,1,(char *)&buf);
	else writeblock(fd,2,(char *)&buf);
}
